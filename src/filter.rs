...
pub fn apply_blur(surface: &mut gdk::cairo::ImageSurface, radius: f32, n: usize) -> Result<gdk::cairo::ImageSurface> { //TODO optimize (maybe remove some iter.collect but i'm too lazy to do it)
    if radius <= 0.0 {
        return Ok(surface.clone());
    }
    
    //TODO ultimate optimization: use gles
    ...
    let mut surface_data = surface_data
        .chunks_exact(4).map(|val| val.try_into().unwrap())
        .collect::<Vec<[u8; 4]>>();
    
    gaussian_blur(&mut surface_data,width as usize,height as usize, radius, n);
...
pub fn gaussian_blur(data: &mut Vec<[u8;4]>, width: usize, height: usize, blur_radius: f32,n: usize)
{
    let boxes = create_box_gauss(blur_radius, 5);
